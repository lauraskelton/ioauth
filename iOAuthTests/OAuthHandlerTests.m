...
- (void)testHandleResponseWithData {
    ...
    if (![[[NSUserDefaults standardUserDefaults] objectForKey:kiOAuthAccessTokenKey] isEqualToString:@"someaccesstoken"]) {
        XCTFail(@"OAuthHandler handleResponseWithData does not save access token to user defaults");
        return;
    }
    
    if (![[[NSUserDefaults standardUserDefaults] objectForKey:kiOAuthRefreshTokenKey] isEqualToString:@"somerefreshtoken"]) {
...
- (void)testAccessTokenRequest {
    ...
    if (![[[OAuthHandler sharedHandler] accessTokenRequest].URL.absoluteString isEqualToString:[[OAuthHandler sharedHandler] thisTokenURL]]) {
        NSLog(@"access token request url: %@", [[OAuthHandler sharedHandler] accessTokenRequest].URL.absoluteString);
        XCTFail(@"OAuthHandler accessTokenRequest URL is not correct token url");
        return;
    }
